...
void Initializer::Initialize(const double timestamp, bool enable_imu_updater, State* init_state) {
    ...
    // IMU bias.
    init_state->gyro_bias.bg.setZero();
    init_state->gyro_bias.state_idx = state_idx;
    state_idx += 3;
    init_state->acc_bias.ba.setZero();
    init_state->acc_bias.state_idx = state_idx;
    state_idx += 3;
