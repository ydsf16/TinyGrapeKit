...
Eigen::Matrix3d ComputeMvv(const Eigen::Matrix3d &Mav, const Eigen::Vector3d &velocity, const Eigen::Vector3d &Wnie, const Eigen::Vector3d &Wnen) {
    return velocity.cross(Mav) + SkewMat(2.0 * Wnie + Wnen);
}
...
KFMat ErrorModel::ComputeFMatrix(const InsState &ins_state) {
    ...
    const Eigen::Vector3d fbsf = ins_state.acc;
    ...
    F.block<3, 3>(kAttErrIdx, kVelErrIdx) = ComputeMav(Rmh, Rnh, latitude);
    ...
    F.block<3, 3>(kVelErrIdx, kVelErrIdx) = ComputeMvv(ins_state.velocity, ins_state.Wnie, ins_state.Wnen);
...
Eigen::Matrix<double, kKFStateDim, 6> ErrorModel::ComputeGMatrix(const InsState &ins_state) {
    Eigen::Matrix<double, kKFStateDim> G;
    G.setZero();
